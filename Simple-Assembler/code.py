...
def assembly_to_binary(l,c):
    ...
    s = c[-1]
    search = list(map(str,s.split()))

    if "hlt" not in search:
        Error=1
    ...
    if (type_A.count(line[0]) > 0):
        ...
        if(line[0] == "ld"):
            ...
            if (line[2] in var_list):
                bin_  = bin_ + dictionary[line[1]]
                mem_ad= bin(len(c) + var_list.index(line[2]))
                mem_ad = mem_ad[2:]
                a = mem_ad[::-1] 
                while len(a) < 8:
                    a += '0'
...
#            elif (int(line[2],2) == int and int(line[2],2)<=len(c)):            
#                bin_  = bin_ + dictionary[line[1]]
#                mem_ad= line[2]
#                bin_  = bin_ + str(mem_ad)
            elif(line[2] not in var_list) :
                Error =1
                return("Use of undefined variables")
        ...
        for i in c :
            parts = list(map(str,i.split()))
            if line[1] == parts[0]:
                label_ad = bin(c.index(i)).replace('0b','')
                w = label_ad[::-1]
                while len(w)<8:
                    w = w+"0"
                label_ad = w[::-1]
